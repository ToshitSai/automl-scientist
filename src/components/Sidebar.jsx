...
export default function Sidebar({ 
  ...
  const renderProjectItem = (proj) => {
    ...
    return (
      ...
      >
        <span className="truncate max-w-[170px]">{proj.name.replace(/^Research:\s*/, '')}</span>
    ...
    <aside className="w-[270px] shrink-0 h-screen bg-[#0D111A] border-r border-[#1E293B] flex flex-col justify-between select-none">
      ...
      <div className="p-4 border-b border-[#1E293B]/60 flex flex-col gap-4">
        ...
        <button
          ...
          className="w-full py-2.5 px-3 rounded-lg bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 text-xs font-medium flex items-center justify-center gap-2 transition-all hover:border-cyan-400/50 cursor-pointer shadow-sm"
        >
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-[#131822] border border-[#212B3B] rounded-lg pl-8 pr-3 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-cyan-500/50"
          />
        </div>
      </div>
        ...
        {projects.length === 0 ? (
          ...
          <>
            {groups.today.length > 0 && (
              <div>
                <div className="text-[10px] font-mono tracking-wider text-slate-500 uppercase px-2 mb-1.5 font-semibold">
                  Today
                </div>
      ...
      <div className="p-3 border-t border-[#1E293B] space-y-1 bg-[#090D14]">
        <button
          onClick={onOpenSettings}
          className="w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs text-slate-300 hover:text-slate-100 hover:bg-[#161B26] transition-all"
        >
        ...
        {/* Isolation System Badge */}
        <div className="px-3 py-2 text-[10px] font-mono text-slate-400 bg-[#121722] rounded-lg border border-[#1E293B] flex items-center justify-between">
          <span className="flex items-center gap-1.5">
            <span className={`w-1.5 h-1.5 rounded-full ${dockerReady ? 'bg-emerald-400' : 'bg-cyan-400'}`}></span>
            {dockerReady ? 'Docker Sandbox' : 'Process Sandbox'}
          </span>
          <span className="text-slate-500">{dockerReady ? 'Isolated' : 'Subprocess'}</span>
        </div>
      </div>
    </aside>
  );
